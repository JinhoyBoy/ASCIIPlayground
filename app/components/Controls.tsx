...
export default function Controls({
  ...
  const clamp = (v: number) => Math.max(40, Math.min(200, v));
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          value={columnCount}
          onChange={(e) => onColumnCountChange(Number(e.target.value))}
          onBlur={(e) => onColumnCountChange(clamp(Number(e.target.value)))}
          className="w-full rounded border border-zinc-600 bg-zinc-800 px-2 py-1 text-sm text-zinc-200"
        />
      </div>
    </div>
  );
